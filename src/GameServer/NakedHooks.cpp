...
{
	// Inicialização dos hooks automatizados
	if (!DeactiveOriginals::Initialize()) return false;

	// Controle da load/save user, implementado para que seja possível adicionar a extra.
	if (!CAccount::Initialize()) return false;

	// Controle de pacotes enviados/recebidos
	...
	//Desabilita um limite de nível para que seja possível dropar em monstros
	...
	// Bonus de experiência em area/classe
	PEHook::JMP_NEAR(0x056FA46, CExpController::NKD_ExpMultiply, 7);

	//Fix acentos gritar/chat comum
	PEHook::JMP_NEAR(0x054BA7B, (void*)0x054BA8D, 4);
	PEHook::JMP_NEAR(0x054FA01, (void*)0x054FA13, 4);
